...
const DeliveryZonesEditForm = ({
  lat,
  lng,
  setLat,
  setLng,
  deliveryZonesEditForm,
  setDeliveryZonesEditForm,
  deliveryZonesEditFormNumber
}) => {
  ...
  const onSubmit = (data) => {
    updateData(
      `/restaurant/delivery-zone?_id=${_id}`,
      { ...data, lat, lng },
      admin?.token
    ).then((res) => {
      if (res.success) {
  ...
  return (
    <>
      <div className=" fixed top-0 left-0 w-full h-screen bg-[#11111185] ">
        <div
          onClick={() => setDeliveryZonesEditForm(!deliveryZonesEditForm)}
        ...
        <div className="fixed top-0 right-0 w-[60%] h-screen bg-[#fff] py-8 px-7  overflow-y-auto  ">
          <div className="flex items-center justify-between mb-4 border-b border-solid border-[#D7D7D7]  pb-3">
            <h4 className=" text-[#757575] text-md ">Delivery Zones Edit</h4>
            <button
              onClick={() => setDeliveryZonesEditForm(!deliveryZonesEditForm)}
...
export default DeliveryZonesEditForm;